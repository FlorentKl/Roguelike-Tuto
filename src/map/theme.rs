...
fn get_tile_glyph_default(idx: usize, map: &Map) -> (u8, RGB, RGB) {
    let glyph: u8;
    let mut fg: RGB;
    let mut bg: RGB = RGB::from_f32(0.0, 0.0, 0.0);
    ...
    if !map.visible_tiles[idx] {
        fg = fg.to_greyscale();
        bg = RGB::from_f32(0.0, 0.0, 0.0); //No stains out of visual range
    }
...
fn wall_glyph(map: &Map, x: i32, y: i32) -> u8 {
    ...
    //Next, we check each of the 4 directions and add to the mask.
    //We're adding numbers corresponding to each of the first four bits in binary - so 1,2,4,8.
    //This means that our final number will store whether or not we have each of the four
    //possible neighbors.
    //For example, a value of 3 means that we have neighbors to the north and south.
