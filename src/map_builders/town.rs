...
pub fn town_builder(
    ...
    let mut chain = BuilderChain::new(new_depth, width, height, "The Town of Creation");
...
impl TownBuilder {
    ...
    ///Return building defined by a vector [x1, y1, x2, y2]
    fn buildings(
        &mut self,
        rng: &mut rltk::RandomNumberGenerator,
        build_data: &mut BuilderMap,
        ...
        while n_buildings < 12 {
            let bx = rng.roll_dice(1, build_data.map.width - 32) + 30; //x
            let by = rng.roll_dice(1, build_data.map.height) - 2; //y
            let bw = rng.roll_dice(1, 8) + 4; //x2
            let bh = rng.roll_dice(1, 8) + 4; //y2
        ...
        /*
        // Outline buildings
        //Not useful because of edge aremarked as wall
    ...
    fn add_paths(&mut self, build_data: &mut BuilderMap, doors: &[usize]) {
        ...
        for door_idx in doors.iter() {
            ...
            for r in roads.iter() {
                let mut distance = rltk::DistanceAlg::PythagorasSquared.distance2d(
                    door_pt,
                    rltk::Point::new(
                ...
                //Add weight to choose to go to main roads by diminishing distance
                if main_roads.contains(r) {
                    distance *= 0.8;
                }

                nearest_roads.push((*r, distance));
            }

            //Sort road from the smallest distance to highest
            nearest_roads.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());

            let destination = nearest_roads[0].0; //choose first sorted road, which is the smallest
            let path =
                rltk::a_star_search(*door_idx as i32, destination as i32, &mut build_data.map);
            if path.success {
    ...
    fn sort_buildings(
        ...
        //Sort building by size in descending order (b.cmp(a))
        let mut building_size: Vec<(usize, i32, BuildingTag)> = Vec::new();

        //building.0 : x1, building.1 : y1, building.2 : x2, building.3 : y2
        for (i, building) in buildings.iter().enumerate() {
            let building_area =
                i32::abs(building.0 - building.2) * i32::abs(building.1 - building.3);
            building_size.push((i, building_area, BuildingTag::Unassigned));
        }
    ...
    fn building_factory(
        ...
        for (i, building) in buildings.iter().enumerate() {
            ...
            match build_type {
                BuildingTag::Pub => self.build_pub(rng, &building, build_data),
                BuildingTag::Temple => self.build_temple(rng, &building, build_data),
                BuildingTag::Blacksmith => self.build_smith(rng, &building, build_data),
                BuildingTag::Clothier => self.build_clothier(rng, &building, build_data),
                BuildingTag::Alchemist => self.build_alchemist(rng, &building, build_data),
                BuildingTag::PlayerHouse => self.build_player_house(rng, &building, build_data),
                BuildingTag::Hovel => self.build_hovel(rng, &building, build_data),
                BuildingTag::Abandoned => self.build_abandoned(rng, &building, build_data),
                _ => {}
            }
        }
    }
    ...
    ///Build the pub and place the player inside
    fn build_pub(
        &mut self,
        rng: &mut rltk::RandomNumberGenerator,
        building: &(i32, i32, i32, i32),
        ...
        //Place the player
        let start_x = building.0 + (building.2 / 2);
        let start_y = building.1 + (building.3 / 2);
        build_data.starting_position = Some(Position {
            x: start_x,
            y: start_y,
        });
        let player_idx = build_data.map.xy_idx(start_x, start_y);
        ...
        self.random_building_spawn(rng, &building, build_data, &mut to_place, player_idx);
    }
    ...
    fn build_temple(
        ...
        self.random_building_spawn(rng, &building, build_data, &mut to_place, 0);
    }
    ...
    fn spawn_townsfolk(
        ...
        //tiles we know won't be inside of a building, because we removed them when we placed buildings
