...
impl Tooltip {
    ...
    fn render(&self, ctx: &mut Rltk, x: i32, y: i32) {
        ...
        let coord_x: i32 = if x < 0 { 0 } else { x };
        let coord_y: i32 = if y < 0 { 0 } else { y };
        ...
        for (i, s) in self.lines.iter().enumerate() {
            ...
            ctx.print_color(coord_x + 1, coord_y + i as i32 + 1, col, black, &s);
        }
    }
}
...
pub fn main_menu(gs: &mut State, ctx: &mut Rltk) -> MainMenuResult {
    ...
    let save_exists = system_saveload::does_save_exist();
...
pub fn draw_ui(ecs: &World, ctx: &mut Rltk) {
    ...
    match hc.state {
        ...
        HungerState::Normal => {
            ctx.print_color(50, 44, RGB::named(rltk::LIGHT_GREEN), black, "Fine")
        }
...
fn draw_tooltips(ecs: &World, ctx: &mut Rltk) {
    ...
    if mouse_pos.0 < 40 {
        // Render to the left
        arrow = to_cp437('→');
        arrow_x = i32::min(0, i32::abs(mouse_pos.0 - 1));
    } else {
        // Render to the right
        arrow = to_cp437('←');
...
fn draw_menu<S: ToString>(
    ...
    match menutype {
        ...
        MenuType::RemoveItem => {
            for (entity, pack, name) in (&entities, &equipped, &names).join() {
                match &pack.slot {
                    EquipmentSlot::Melee => {
                        ctx.print(18, y, "Melee");
                    }
                    EquipmentSlot::Shield => {
                        ctx.print(18, y, "Shield");
                    }
                    EquipmentSlot::Head => {
                        ctx.print(18, y, "Head");
                    }
                    EquipmentSlot::Torso => {
                        ctx.print(18, y, "Torso");
                    }
                    EquipmentSlot::Hands => {
                        ctx.print(18, y, "Hands");
                    }
                    EquipmentSlot::Legs => {
                        ctx.print(18, y, "Legs");
                    }
                ...
                ctx.print(21, y + 1, &name.name.to_string());
                equippable.push(entity);
                y += 2;
                j += 1;
            }
...
pub fn show_inventory(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_menu(gs, ctx, "Inventory", MenuType::Inventory)
}

pub fn show_drop_menu(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_menu(gs, ctx, "Drop menu", MenuType::Inventory)
}

pub fn remove_item_menu(gs: &mut State, ctx: &mut Rltk) -> (ItemMenuResult, Option<Entity>) {
    draw_menu(gs, ctx, "Remove which item ?", MenuType::RemoveItem)
}
